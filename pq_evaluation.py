...
def main(args):
    ...
    # For PQ it is important to make sure the query descriptors are normalized as well
    queries /= np.sqrt((queries * queries).sum(axis=1))[:, None]
    ...
    print('--> Build PQ index and evaluate')

    tree_lib.pq_evaluation(query_indices, query_id_to_positive_classes, view_info, queries, dataset, args.pq_num_bytes)
